...
def train(
    ...
    def _train(runner_state: RunnerState):
        def epoch_fn(runner_state: RunnerState, _unused):
            ...
            update_state = UpdateState(train_state, samples, advantages, targets, key)
            ...
            def callback(info_and_epoch_idx):
                ...
                print(f"Epoch: {epoch_idx}, " f"Return: {return_values.mean():.2f}")
        ...
        # run epochs
        runner_state, metric = jax.lax.scan(epoch_fn, runner_state, None, num_epochs)
